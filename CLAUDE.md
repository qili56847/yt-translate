...
5. **[review]** — optional human review step: CLI `--review` pauses for manual editing; Web UI shows inline SRT editor with save/skip/continue
6. **synthesize** (`steps/synthesize.py`) — Edge-TTS generates Chinese speech per segment with fixed +40% rate, bidirectional time-alignment (slowdown/speedup/truncation), mixes into single voice track
7. **compose** (`steps/compose.py`) — ffmpeg mixes voice track with background audio, burns Chinese subtitles (libass), outputs final MP4
...
**Key utilities:** `utils/progress.py` (CLI/SSE dual output), `utils/srt.py` (SRT parsing + `merge_segments` fragment consolidation), `utils/audio.py` (ffmpeg wrapper)
...
- **Human review gate**: `pipeline.py` accepts `review_callback`; CLI uses `input()` blocking, Web UI uses `threading.Event` with 1-hour timeout (timeout = task cancelled)
